...
grid.agg <- function(data2agg=NULL, region.map=NULL, agg.function= "production", weight.map=NULL, crop_name=NULL){
    ...
    if(agg.function %in% c("summary")){
        ...
        if(agg.function %in% c("weighted.m.custom")){
            ...
            if(agg.function %in% c("weighted.m")){
            ...
            }else{## AREA WEIGHTED AVERAGE BEHAVIOR:
                if(agg.function %in% c("production")){
            ...
            }else{stop("A proper agg.function must be specified. Acceptable values are: 'production', 'weighted.m', 'summary', and 'weighted.m.custom'")
              }}}}
agg
}
